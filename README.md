...
# [HW5](https://colab.research.google.com/drive/1ybh6-o59V4H0ImK7XGocARHDbzMYMwOu?usp=sharing)
## 中文文本斷詞與命名實體識別（NER）作業介紹
本專案選用一段關於科技業新聞的中文文本，內容講述輝達執行長黃仁勳訪台參加COMPUTEX的相關消息。透過 jieba 進行中文斷詞，並使用 stanza 進行命名實體識別（NER），分析文本中出現的重要詞彙與實體。

我們利用詞頻統計繪製高頻詞柱狀圖，呈現文本中的核心詞彙；同時也對命名實體進行類型分類並繪製分佈圖，以視覺化方式展現文本中的人名、地名、組織等實體分布情況。

## 反思與心得
在斷詞與NER的過程中，我發現科技新聞中摻雜大量專有名詞（如公司名、產品名、技術名）對模型是一大挑戰，斷詞工具對新詞和英文縮寫的識別還有改進空間。NER模型雖能辨識多數常見實體，但在特定領域詞彙的精準度上仍有限。

透過這次練習，我深刻體會到中文文本處理的複雜性及斷詞準確度的重要性。若將來將此技術應用於新聞摘要、自動標註或輿情監控等實務，必須搭配領域特定詞庫及持續優化模型，以提升效果與實用性。
